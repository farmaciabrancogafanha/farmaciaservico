...
- Sem frameworks, sem bundler. Únicos serviços externos: GoatCounter (analytics, ver notas de infraestrutura) e api.qrserver.com (imagens dos QR codes)
...
## Cartões de farmácia (partilhados)

A página principal e a página `/farmacias` usam o mesmo cartão, gerado por `htmlCartaoFarmacia(f, slug, ehIlhavo, deServico)`:

- Nome da farmácia seguido do grupo de botões `.acoes-nome` (inline-flex, gap 8px): "📍 Ver no mapa" (link do catálogo para o Google Maps) e, apenas no cartão da Branco, "🕒 Horário desta semana" (link absoluto para o `/farmaciabranco/horario` canónico). O grupo nunca se separa: quando não cabe ao lado do nome, desce inteiro para a linha seguinte.
- Morada.
- Um botão "📞 Ligar" por número de telefone, com a nota legal de custo da chamada por baixo (ver notas de infraestrutura). A distinção fixo/móvel é automática: números a começar por `+3519` contam como móveis.
- `deServico = true` acrescenta a classe `.de-servico` (fundo de destaque), usada na página `/farmacias`.

### Botão "Ligar" condicional (Ílhavo)

Os botões "Ligar" dos números fixos das farmácias de Ílhavo têm a classe `.ligar-condicional` e são escondidos fora da janela das 9h00 às 21h15 (15 minutos de tolerância depois do fecho da disponibilidade), pela função `atualizarVisibilidadeLigar`, chamada a cada tick. Números móveis e todos os números de Aveiro (serviço 24h) ficam sempre visíveis. O esconder usa `el.style.display` explícito, não o atributo `[hidden]`, que não vence regras CSS de `display`. A função guarda o último estado em cache e só toca no DOM quando o estado muda (duas vezes por dia); o cache é invalidado quando os cartões são re-renderizados na mudança de turno.
...
- Cada página tem no fundo uma linha de diagnóstico com o formato `LxA dpr:N mq:breakpoint` (ex.: `1024×575 dpr:2 mq:médio`), **única fonte fidedigna** para confirmar a configuração corrente. O canvas lógico é `largura_CSS × DPR` e não aparece na linha; calcula-se de cabeça. Aparece no rodapé de todas as páginas: site principal, horário, estatística, mapa e farmácias. Existe ainda um overlay de diagnóstico fixo no canto do ecrã, activado com `?debug=1` na URL, para quando o rodapé não está visível (ex.: TV em fallback)
- Duas TVs na montra:
...
### Rodapé do overlay do horário

Três colunas alinhadas pelo topo, classes prefixadas `fo-` (footer-overlay) para não colidirem com o CSS do rodapé do site principal:
- Esquerda: balão "VOCÊ ESTÁ AQUI" + QR + link da própria página do horário
- Centro: QR + link da raiz do site, e a linha de diagnóstico de viewport por baixo
- Direita: QR + link do Linktree da Branco; escondida nos hostnames de parceiros

Os QR codes são gerados em runtime via api.qrserver.com, com o URL construído a partir do `location.origin` corrente (fallback para o URL canónico em `file://`).
...
- No canto superior direito, empilhados e com aparência homogénea, os botões de navegação: "Mapa do Site" e "Horário desta semana" (links, sempre visíveis) e "Tabela" (regresso à listagem, visível apenas nas vistas de detalhe de semana, mês ou ano).
...
## Página de farmácias (`/farmacias`)

Overlay com a lista de todas as farmácias dos dois concelhos, em duas colunas (Aveiro à esquerda, Ílhavo à direita; empilham abaixo de 760px). Os cartões são os mesmos da página principal (`htmlCartaoFarmacia`). A farmácia de serviço de hoje recebe o destaque `.de-servico`, sem mudar de posição na lista.

A ordem é fixa, pela sequência real do serviço (1ª volta do ano na escala), rodada para começar na farmácia de nome alfabeticamente mais baixo:

- Aveiro: uma sequência única.
- Ílhavo: dois grupos, primeiro os arredores (`branco`, `anca_castro`, `ribau`, `morais`) e depois a cidade (`moderna`, `senos`, `do_cais`, `santos`), cada grupo com a sua sequência. A divisão reflecte o ciclo de 8 dias da escala de Ílhavo, confirmado empiricamente sem excepções: dias com 1 farmácia de serviço são sempre da cidade; dias com 2 são sempre 1 dos arredores + 1 da cidade.
- Farmácias do catálogo fora da escala vão para o fim, por ordem alfabética.

Ao contrário das rotas `/farmaciabranco/*`, a `/farmacias` não exibe a marca Branco e mantém-se acessível nos domínios de parceiros.
...
O Decreto-Lei 59/2021, revisto pela Lei 14/2023 (aplicável desde 7 de Abril de 2023), obriga a indicar, junto de cada número de contacto no sítio da Internet, o custo da chamada ou, em alternativa, o tipo de rede. Cada número nos cartões de farmácia tem por isso um `<span class="nota-custo-chamada">` com a frase "Chamada para a rede fixa nacional" (números fixos/geográficos) ou "Chamada para a rede móvel nacional" (móveis, `+3519`). A frase é tipograficamente discreta (10px, cinzento claro, sem negrito) e é escondida no modo montra. A distinção fixo/móvel é automática no render, em `htmlCartaoFarmacia`.

### Hostnames especiais

Dois mecanismos, ambos dependentes de `location.hostname`:

1. **Linktree escondido.** A constante `HOSTNAMES_SEM_LINKTREE` (no JS) lista os hostnames onde o bloco do Linktree da Branco é escondido, tanto no rodapé do site principal (`#footer-direita`) como no rodapé do overlay do horário (`.fo-direita`). O default é mostrar; esconder é opt-in explícito por hostname. O esconder usa `style.display = "none"`.
2. **Estanquidade de marca.** Nos mesmos hostnames, qualquer rota `/farmaciabranco/*` é redireccionada para a raiz com `location.replace("/")` logo no arranque, antes de criar qualquer overlay. Os sites de parceiros nunca mostram páginas com a marca Farmácia Branco. A rota `/farmacias` não é afectada e funciona em todos os domínios.

### Analytics (GoatCounter)

O `<head>` carrega o script do GoatCounter (`gc.zgo.at/count.js`, async), que conta as visitas em `farmaciabrancogafanha.goatcounter.com`. Sem cookies e sem dados pessoais, por isso não exige banner de consentimento. Se o script falhar, o resto do site não é afectado.
